...
比如数据写到了master，但是还没有同步到slave，然后master宕机了，这时候在slave上读不到。
...
脑裂，其实就是由于网络分区或者是短暂宕机后，同时出现两个`master`的情况。

这样的情况只能通过设置master的最小slave数量，这样在原来的master宕机恢复后由于变成了slave，那么会更早拒绝客户端的写入请求。

#### 备份恢复

##### RDB

创建快照的方式对数据进行保存，可以设置`保存点`（类似触发器）来实现，即设置多少个key值改变后进行保存。

创建快照的时候是主进程会fork出一个子进程来完成这个复制快照的操作，先将数据写入临时文件，写入成功后，再替换之前的文件，用二进制压缩存储。避免了主线程的阻塞。

同时因为是`RDB`备份的一个快照文件，可以加密后远程/异地备份，安全性和可靠性更好。

##### AOF

`Ahead of forward`, 采用追加操作日志（只记录写、删除操作，查询操作不会记录）的方式，将每次对key的操作直接追加到日志内，所以日志其实是可读的（毕竟就是对`redis`的操作语句），

并会对相关的`redis`操作做优化，有点像是`WAL`的做法。

通常使用以下两种模式：

- `always`，每一次操作都追加日志，性能更影响大。:dog:
- `everysec`，每秒保存一次，后台子线程调用。



如果对数据依赖比较高会是`RDB`和`AOF`一起使用，来保证数据的可靠性。

如果持久化依赖十分高的话，`Redis`可能就不适合你了，更好的做法是落库。



## 参考

[官方文档-Redis cluster tutorial](https://redis.io/topics/cluster-tutorial)

[Redis系列文章](https://juejin.im/post/6847902224144662542)

[Redis集群方案怎么做](https://www.zhihu.com/question/21419897)

[一致性Hash与Redis集群数据分片- JedisCluster实现](https://www.cnblogs.com/zhaochunxue/p/13326933.html)
